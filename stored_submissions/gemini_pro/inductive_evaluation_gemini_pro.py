...
def get_all_d_datasets():
    """Get all D_ prefixed datasets from BESESR"""
    ...
    print(f"Found {len(test_datasets)} D_ datasets to process")
...
def create_inductive_prompt(essay_text: str, dataset_info: dict) -> dict:
    ...
    question = dataset_info.get('question', '')  # Get question from dataset_info
    
    # Determine if categorical based on dataset name
    is_3way = '3way' in dataset_name.lower()
    is_2way = '2way' in dataset_name.lower()
    is_categorical = is_3way or is_2way
    
    if is_3way:
        # 3-way classification: correct/contradictory/incorrect
        examples = """
EXAMPLE 1:
...
def load_essays_from_dataset(dataset_name: str, num_essays: int = None):
    """Load essays from BESESR (not directly from HuggingFace)"""
    try:
        
        tester = SingleModelTester()
        
        # Download test data from BESESR
        df = tester.download_test_data(dataset_name, num_essays)
        if df is None:
            print(f"  Failed to download from BESESR")
            return []
        
        # Prepare essays using BESESR's method
        essays_raw = tester.prepare_essays_for_prediction(df, dataset_name)
        
        # Convert to format needed for inductive evaluation
        essays = []
        for essay_raw in essays_raw:
            ...
            # Get essay_set for ASAP-AES (needed for correct range)
...
def run_inductive_evaluation(api_key):
    """Run inductive evaluation on all D_ datasets"""
    
    client = get_client(api_key)
    
    print("="*70)
    print("INDUCTIVE REASONING EVALUATION - Gemini 2.5 Pro")
    print("="*70)
    print(f"Starting at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    all_datasets = get_all_d_datasets()
    if not all_datasets:
    ...
    for dataset_idx, dataset_name in enumerate(all_datasets, 1):
        ...
        try:
            ...
            dataset_result['total_cost'] = dataset_cost
            ...
            print(f"  ✓ Completed: {dataset_result['essays_evaluated']} essays, Cost: ${dataset_cost:.6f}")
    ...
    all_results['total_cost'] = total_cost
    all_results['successful_datasets'] = successful_datasets
    
    print("\n" + "="*70)
    print("INDUCTIVE EVALUATION COMPLETE")
    print("="*70)
    print(f"Successful datasets: {successful_datasets}/{len(all_datasets)}")
    print(f"Total cost: ${total_cost:.6f}")
    
    timestamp = int(time.time())
    filename = f"inductive_Gemini 2.5 pro_all_datasets_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    # Accept API key as command-line argument or from environment
    if len(sys.argv) > 1:
        api_key = sys.argv[1]  # Passed by parallel runner
        print(f"Using API key from command-line argument")
    else:
        api_key = os.getenv("OPENROUTER_API_KEY")  # Manual run
        print(f"Using API key from OPENROUTER_API_KEY environment variable")
    
    if not api_key:
        print("ERROR: No API key provided")
        print("Either set OPENROUTER_API_KEY environment variable or pass key as argument")
        sys.exit(1)
    
    print("\nThis will process ALL D_ datasets with ALL essays")
    print("Estimated: 30,000+ API calls")
    print("Estimated cost: $50-500+ depending on pricing")
    print("Estimated time: 10-20 hours")
    
    confirm = input("\nProceed with full inductive evaluation? (y/n): ").strip().lower()
    if confirm == 'y':
        run_inductive_evaluation(api_key)
    else:
