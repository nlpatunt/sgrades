// Fully-trained Leaderboard JavaScript
const API_BASE_URL = '';

// DOM Elements
const elements = {
    leaderboardTable: document.getElementById('leaderboard-fully-trained-table'),
...
// ===== Load Fully-trained Leaderboard =====
async function loadFullyTrainedLeaderboard() {
    if (!elements.leaderboardTable) return;
    
    showLoading(elements.leaderboardTable, 'Loading fully-trained leaderboard...');
    
    try {
        const metric = elements.metricSelector?.value || 'avg_quadratic_weighted_kappa';
        
        console.log('Debug: Loading fully-trained leaderboard with metric:', metric);
        
        const data = await fetchAPI('/api/submissions/leaderboard?methodology=fully-trained&metric=' + metric + '&limit=20');
        
        console.log('Debug: API response:', data);
        
        if (!data || !data.rankings || data.rankings.length === 0) {
            const actionHtml = '<a href="index.html#submit" class="btn btn-primary"><i class="fas fa-rocket"></i> Submit Fully-trained Benchmark</a>';
            showEmptyState(
                elements.leaderboardTable,
                'No Fully-trained Benchmarks Yet',
                'Be the first to submit a fully-trained benchmark to appear on this leaderboard!',
                actionHtml
            );
            return;
        }
        ...
        console.log('Success: Loaded fully-trained leaderboard with ' + data.rankings.length + ' entries');
        
    } catch (error) {
        showError(elements.leaderboardTable, 'Failed to load fully-trained leaderboard. Please check the API connection.');
        console.error('Failed to load fully-trained leaderboard:', error);
    }
}
...
function buildLeaderboardTable(data, tableElement) {
    ...
    tableHTML += '<p><strong>Fully-trained Models:</strong> These models were trained on the provided training/validation splits before evaluation on test data.</p>';
    tableHTML += '</div>';
...
function setupEventListeners() {
    if (elements.metricSelector) {
        elements.metricSelector.addEventListener('change', loadFullyTrainedLeaderboard);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Fully-trained Leaderboard Page Initialized');
    setupEventListeners();
    loadFullyTrainedLeaderboard();
    
    // Auto-refresh every 5 minutes
    setInterval(loadFullyTrainedLeaderboard, 300000);
});