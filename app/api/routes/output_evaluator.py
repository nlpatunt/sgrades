...
def evaluate_output_submission(submission_id: int, dataset_name: str, file_path: str, file_format: str):
    ...
    """
    try:
        print(f"📂 Loading submission file for {dataset_name} at {file_path}")
        ...
        if "essay_id" not in df.columns or "predicted_score" not in df.columns:
            raise ValueError("File must contain columns: 'essay_id' and 'predicted_score'")

        # Load ground truth
        gt = dataset_manager.load_ground_truth_scores(dataset_name)
        if not gt:
            raise ValueError(f"Ground truth not available for dataset: {dataset_name}")

        gt_df = pd.DataFrame(gt)  # Should contain essay_id, human_score, score_range
        ...
        merged["normalized_pred"] = merged.apply(
            lambda row: normalize_score(row["predicted_score"], (1, 6), (row["score_range"][0], row["score_range"][1])),
            axis=1
        )
        ...
        print(f"✅ Evaluation complete for submission {submission_id} on {dataset_name}")
